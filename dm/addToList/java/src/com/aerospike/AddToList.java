...
public class AddToList {
	public static void appendOrdered(AerospikeClient client, int value) {
		...
		if(value == 10){  //First item, idenitfy List Order. 
			//Extra Credit A) Try changing 10 to 11!
			client.operate(null, key, ListOperation.append(lPolicy, "myList", Value.get(value)));                
		} 
		else  {
			client.operate(null, key, ListOperation.append("myList", Value.get(value)));                
		} 
	}
	...
	public static void appendUnordered_setOrder(AerospikeClient client, int value) {
		...
		ListPolicy lPolicy = new ListPolicy(ListOrder.UNORDERED, ListWriteFlags.DEFAULT);                 
		if(value != 0){  //First loop items, identify List Order. UNORDERED is also DEFAULT
			client.operate(null, key, ListOperation.append(lPolicy, "myList", Value.get(value)));                
		} 
		else {
			client.operate(null, key,   //Extra Credit B) Comment next line, setOrder(), and retry. 
	...
	public static void appendOrdered_insertUnordered(AerospikeClient client, int value) {
		...
		ListPolicy lPolicy = new ListPolicy(ListOrder.ORDERED, ListWriteFlags.DEFAULT);                 
		if(value == 10){  //First item sets List Order as ORDERED.
			client.operate(null, key, ListOperation.append(lPolicy, "myList", Value.get(value)));                
		} 
		else if(value > 10)  {
			client.operate(null, key, ListOperation.append("myList", Value.get(value)));                
			...
			client.operate(null, key, ListOperation.setOrder("myList", ListOrder.UNORDERED),
