...
public class TweetService {
	...
	public void queryUsers() throws AerospikeException {
	    ...
	    // NOTE: The recommended way of creating indexes in production env is via AQL.
			...
			stmt.setFilters(Filter.geoWithinRadius("location", new Double(lng).doubleValue(), 
					new Double(lat).doubleValue(), new Double(radius).doubleValue()));
				
			rs = client.query(null, stmt);
			while (rs.next()) {
				Record r = rs.getRecord();
				console.printf("Tweet By: " + r.getValue("username").toString() + "\n");
				console.printf("Tweet: " + r.getValue("tweet").toString()+"\n");
			}
    ...
    public void queryOffers() throws AerospikeException {
	    ...
	    // NOTE: The recommended way of creating indexes in production env is via AQL.
			...
			Statement stmt = new Statement();
			stmt.setNamespace("test");
			stmt.setSetName("offers");
			stmt.setIndexName("geo_offers_index");
			...
			while (rs.next()) {
				Record r = rs.getRecord();
				console.printf("Mall: " + r.getValue("mall").toString() + "\n");
				console.printf("Shop: " + r.getValue("shop").toString() + "\n");
				console.printf("Discount: " + r.getValue("discount").toString() + "%% \n");
			}
