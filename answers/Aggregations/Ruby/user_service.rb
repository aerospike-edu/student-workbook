...
module Training
  module UserService
    ...
    def aggregate_by_region(client)
      puts "\nAggregate users by region, filter on tweet count in a given range".colorize(:color => :blue, :mode => :bold)
      print "Ensuring the UDF module is registered ≻".colorize(:color => :black, :mode => :bold)
      begin
        register_lua(client, 'udf/aggregationByRegion.lua', 'aggregationByRegion.lua')
        self.yep
      rescue Exception => e
        self.nope
        puts "Failed to register the UDF module".colorize(:color => :red, :mode => :bold)
      ...
      max = gets.chomp.to_i
      # AQL equivalent:
      # AGGREGATE aggregationByRegion.sum() ON test.users WHERE tweetcount BETWEEN _min_ AND _max_
      statement = Statement.new('test', 'users')
      statement.filters << Filter.Range('tweetcount', min, max) unless min == 0 && max == 0
      statement.set_aggregate_function('aggregationByRegion', 'sum')
