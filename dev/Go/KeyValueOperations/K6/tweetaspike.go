...
func UpdatePasswordUsingCAS(client *Client) {
	...
	fmt.Scanf("%s", &username)
        ...
        //Exercise K6 - add code at places marked "x" //K6
	if len(username) > 0 {
		// Check if username exists
		userKey, _ := x //K6
		userRecord, err :=  x //K6
		...
		if err == nil {
			...
			// record generation
			writePolicy.Generation = x //K6 
			writePolicy.GenerationPolicy = x //K6
			// password Bin
			passwordBin := x //K6
...
func updateUser(client *Client, userKey *Key,
	...
	//fmt.Printf("\nINFO: The tweet count now is: %v\n", bin1)
        ...
        //Exercise K6 - Uncomment the call below and pass correct arguments to the function.
	//updateUserUsingOperate(userKey, policy, ts); //K6
}

func updateUserUsingOperate(client *Client, userKey *Key,
	policy *WritePolicy, timestamp int64) {

        //Exercise K6 -add code at "x" //K6
