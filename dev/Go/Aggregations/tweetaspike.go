...
// Exercise A2 - add code at locations marked "x" //A2
func AggregateUsersByTweetCountByRegion(client *Client) {
	...
	// NOTE: UDF registration has been included here for convenience and 
        ...
        //Set LuaPath  - required for running final reduce on client.
        luaPath, _ := os.Getwd()
        luaPath += "/udf/"
        SetLuaPath(luaPath)  //A2 - review


	// Load UDF and wait until UDF is created  
        // A2 - review code to register the Stream UDF
	...
	stmt := NewStatement(x) //A2
	stmt.SetFilter(x) //A2
        //SI on tweetcount is required, we already created in Queries exercise.

	rs, err := client.QueryAggregate(x) //A2
	panicOnError(err)
