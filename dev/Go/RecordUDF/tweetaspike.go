...
func UpdatePasswordUsingUDF(client *Client) {
  ...
  if len(username) > 0 {
    ...
    if userRecord != nil {
      ...
      regTask, err := client.RegisterUDFFromFile(x) //R2 - add code to register UDF
      panicOnError(err)

      // wait until UDF is created  (Not required in production - UDFs are pre-loaded.)
      for {
      ...
      updatedPassword, err := client.Execute(x) //R2
      panicOnError(err)
