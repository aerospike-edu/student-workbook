...
public class ModelsWithExpressions {
    public static void main(String[] args) throws Exception {
      ModelsWithExpressions ce = new ModelsWithExpressions();
      ce.runExample();
      return;
    }
    ...
    private void runExample1(AerospikeClient client) throws Exception {
        ...
        System.out.println("\nRun Example 1 (binExists expression with put) ============");
...
//        wPolicy.durableDelete = true;
        ...
        Expression rexp = Exp.build(
                Exp.gt(Exp.intBin(bin2Name), Exp.val(1))
        );

        //increment and test for rexp condition, return boolean result
        record = client.operate(wPolicy, key,
                Operation.add(bin2),
                Operation.get(bin2Name),
                ExpOperation.read("retval", rexp, ExpReadFlags.DEFAULT)
        );

        System.out.println("***2: " + record);
        System.out.println("***2b: " + record.getValue("retval"));
...
//        System.out.println("update bin3 anyway:             " + record);
    ...
    private void runExample7(AerospikeClient client) throws Exception {
        ...
        //Retrieve LUT into client
        record = client.operate(null, key,
                ExpOperation.read("LUT_val", rexp, ExpReadFlags.EVAL_NO_FAIL)
        );

        System.out.println("LUT : " + record);
